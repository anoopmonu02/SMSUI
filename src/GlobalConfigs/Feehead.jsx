...
function Feehead() {
    ...
    const columns = useMemo(
        () => [
          ...
          {
            accessorKey: 'feehead_name', //access nested data with dot notation
            header: 'Feehead Name',
            size: 150,
          },
          {
            accessorKey: 'feehead_description', //access nested data with dot notation
            header: 'Details',
            size: 250,
          },
        ],
    ...
    const messagetext = "Feehead";

    const savefeehead = async(data) => {
        setError("");
        try{
            console.log(data);
            const res = await customaxios.post('/api/v1/admin/feehead/',{
                feehead_name: data.feehead,
                feehead_description: data.description,
  ...
  return (
    <> 
        <Paper xs={12} elevation={4} sx={{width: '600px', margin: '20px'}}>
            <form onSubmit={handleSubmit(savefeehead)}>
                <Stack spacing={4} p={4}>            
                    ...
                    <MyTextField name="feehead" id="feehead" label="Feehead" 
